...
- `main.go` は `~5 行` 程度: `run(os.Stdin, os.Stdout, os.Stderr)` の結果で `os.Exit`。`run` は同 package の `dispatch.go` に実装。
- **dispatch / handler は `cmd/fumi-host/` に同居** (`package main`)。Host 専用ロジックを module 全体の `internal/` に置くとスコープが広すぎる / CLI から誤 import される余地を完全に消すため、バイナリのディレクトリ配下で閉じる。
- CLI (`fumi scripts run`) は Host を経由せず `internal/runner` を直接叩くので、Host 側の dispatch を CLI から参照する必要はない。

## 2. `main.go` の骨子

```go
// cmd/fumi-host/main.go
package main
...
`run` は同 package の `dispatch.go` に実装する (§10)。
...
| `cmd/fumi-host/` | integration: stdin に JSON を流し stdout をパース、全 op を end-to-end (`package main` テスト) |
...
| `allowed_origins` を Extension ID に固定 | `cmd/fumi/manifest.go` (CLI 側、host 本体は関知しない) |
