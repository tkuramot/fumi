...
```jsonc
{
  "permissions": [
    ...
    "contextMenus",     // fumi.contextMenus.register
...
- 登録中のアクション数
- 最後の取得時刻・最後のエラー
- **「アクション再取得」ボタン**(`getActions` 再実行 → `chrome.userScripts` 更新)

スクリプト編集・閲覧・CRUD 系 UI、DevTools Panel、`fumi.run` の結果自動表示トースト等は**提供しない**。編集はユーザーが自分のエディタでストアを直接操作する。

#### 5.1.3. アクションの登録と実行

- Extension 起動時および Popup「再取得」押下時に `getActions` を呼び、結果を `chrome.userScripts.register` / `update` に反映する。
...
- match / exclude 等のメタデータは、アクションファイル上部の **Tampermonkey 風フロントマター**(`// @key value` 連記)で宣言。Host が `getActions` でメタデータ + 本体を parse して渡す。
...
2. **メソッド名**: 原則 chrome のメソッド名を踏襲。ただし「create + イベントリスナのバインド」のように複数 API を 1 呼び出しにまとめる場合は意味を反映した動詞(`register` 等)に差し替える。
3. **プロパティ名**: chrome の `CreateProperties` / 引数オブジェクトと同名同型(例: `id`, `title`, `contexts`)。
4. **ハンドラ**: chrome のイベント名をそのままプロパティ名に使う(例: `onClicked` は `chrome.contextMenus.onClicked` に対応)。シグネチャも chrome のイベントと一致させる。
5. **スコープ**: MVP で必要なフィールドのみホワイトリスト。未知プロパティは Service Worker 側で捨てる。
6. **例外**: `fumi.run` は `chrome.*` ラッパーではない本機能 API のためルート直下に置く。
...
- **`fumi.*` ヘルパー**:
  ...
  - `fumi.contextMenus.register({ id, title, contexts?, onClicked })` — `chrome.contextMenus.create` + `chrome.contextMenus.onClicked` のバインドを Service Worker 経由で動的登録。`onClicked` のシグネチャは `(info, tab) => void` で `chrome.contextMenus.onClicked` と一致。
...
- `fumi.contextMenus.register` は match するページを開くたびに呼ばれる。Service Worker は**同 id の再登録を冪等に扱い、直近の登録で上書き**する。
...
`chrome.storage` には**キャッシュとステータス情報のみ**(最後の `getActions` 結果、接続エラー情報等)を持つ。
...
1. **`getActions`**: ストアからアクション JS とメタデータを読み出して返す。
2. **`runScript`**: 指定された External Script を起動し、結果を返す。

> **CRUD 系 API は実装しない**(`listScripts` / `readScript` / `writeScript` / `deleteScript` / 設定の読み書き等)。攻撃面を最小化するための強い制約(詳細 §9)。

#### 5.2.4. `getActions` の挙動
...
**サイズ上限**: レスポンス全体が Native Messaging の 1MB 上限を超える場合は `error: { code: "ACTIONS_TOO_LARGE" }` を返す。分割返却は初期版では実装しない(アクション数が多すぎたらファイル分割を見直す指針)。
...
**stdout / stderr のサイズ上限**: 各 1MB(Native Messaging の 1MB 制約からレスポンス全体の上限を逆算)。超過時は `error: { code: "OUTPUT_TOO_LARGE" }` で応答。truncate はしない(部分出力で誤動作するのを避けるため)。大量出力が必要なスクリプトは `$TMPDIR` に書いてパスだけ返す運用をドキュメント化。
...
~/.config/fumi/                       <- ストアルート (既定)
  actions/                            <- アクション JS。chrome.userScripts 用
    my-action.js
  scripts/                            <- External Script。runScript で起動される
    summarize.sh
    open-in-editor.py
```

- `actions/` と `scripts/` を**物理的に分ける**ことで、`runScript` の対象を `scripts/` 配下に限定でき §9 の検証が単純になる。
...
default_timeout_ms = 30000                  # runScript のデフォルトタイムアウト
```
...
**手動のみ**。ストア編集後、Popup「再取得」で `getActions` を再実行して `chrome.userScripts` を更新する。watcher / ポーリングは採用しない(短命 Host では watcher を持てない)。
...
2. Service Worker が `sendNativeMessage` で `getActions` を送信(Host が spawn → 応答 → exit)。
...
5. 対象 URL を開くと User Script が実行され、`fumi.contextMenus.register` 等が Service Worker に届き `chrome.contextMenus` に反映される(dispatch 先は §5.1.4)。
...
4. User Script → Service Worker → Host へ `runScript` 送信(Host spawn)。
...
```jsonc
// Extension → Host
{ "id": "uuid", "op": "getActions" | "runScript", "params": { ... } }
...
| op | params | result |
|---|---|---|
| `getActions` | — | `Action[]`(各要素は `{ id, path, frontmatter, code }`) |
| `runScript` | `scriptPath`, `payload`, `timeoutMs?` | `{ exitCode, stdout, stderr, durationMs }` |

**実装しないオペレーション**(セキュリティ境界として明示): `listScripts`, `readScript`, `writeScript`, `deleteScript`, `renameScript`, `getConfig`, `setConfig`、および任意ファイル I/O。
...
- `getActions` 返却サイズが 1MB を超える場合の扱い。
- 同時実行数の上限。
...
| `writeScript` + `runScript` 連鎖で悪性ファイルを落として実行 | ✗ 封鎖(`writeScript` が存在しない) |
...
4. **Extension 自体の compromise**: 乗っ取られても `getActions` / `runScript` しか使えず、攻撃者は既存のユーザー自身のスクリプトしか叩けない。対策: サプライチェーン管理 **【要確認 EE】**。
...
| Extension ソース | `$(brew --prefix fumi)/share/fumi/extension/` | 開発時 unpacked ロード用 |
...
| `fumi scripts run <name> [--payload '<json>']` | ブラウザを通さず CLI から External Script を叩くデバッグ用。`runScript` と同等の検証を通す |
...
```
fumi/
├── extension/    <- Chrome Extension (Web Store 公開対象)
